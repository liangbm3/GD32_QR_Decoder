...
void DCItoLCD_DMA_Init(uint32_t *DMA_Memory0BaseAddr)
{
	dma_single_data_parameter_struct dma_single_struct; // DMA单传输结构体

	rcu_periph_clock_enable(RCU_DMA1);
	dma_deinit(DMA1, DMA_CH7);
	dma_single_struct.periph_addr = (uint32_t)DCI_DR_ADDRESS;		// 外设基地址
	dma_single_struct.memory0_addr = (uint32_t)DMA_Memory0BaseAddr; // 存储器基地址
	dma_single_struct.direction = DMA_PERIPH_TO_MEMORY;				// 传输方向
	dma_single_struct.number = 65535;								// 65535; // 配置为最大传输数据量
	dma_single_struct.periph_inc = DMA_PERIPH_INCREASE_DISABLE;		// 外设地址不增加
	dma_single_struct.memory_inc = DMA_MEMORY_INCREASE_ENABLE;		// 存储器地址递增
	dma_single_struct.periph_memory_width = DMA_PERIPH_WIDTH_32BIT; // 数据传输宽度
	dma_single_struct.priority = DMA_PRIORITY_HIGH;					// 传输优先级
	dma_single_struct.circular_mode = DMA_CIRCULAR_MODE_DISABLE;	// 单次传输模式
